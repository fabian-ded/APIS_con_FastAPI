...
async def user(id:int):
    return Buscarget(id)#aqui se llama la funcion que se creo para que haga la logica de comprar los id con la url pasada
    
def Buscarget(id: int): #esta es una manera practica para no tener todo en el mismo http y asi es una manera facil de hacerlo y con menos errores  
...
async def user(user: User):#aqui se pasa el usuario a buscar, que venga de la instancia de la clase User.
        ...
        if not found:
            return {"error": f"no se ha encontrado el usuario: {user}"}#si sale error "404"
        else:
